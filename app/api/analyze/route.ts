...
import { fetchRepoMeta, fetchAllCommits, fetchPullRequests, fetchContributors, fetchContributorStats, fetchPackageJson, fetchReadme } from '../../../lib/github';
...
export async function POST(req: NextRequest) {
  try {
        ...
        if (ageMs < ONE_HOUR) {
          ...
          return NextResponse.json({
            ...
            repository: { 
              ...
              name, 
              url: cached.url, 
              description: profile.repoDescription || '', 
              stars: profile.repoStars || 0 
            },
            metrics,
            profile,
            health,
    ...
    const [meta, commits, prs, contributors, stats, packageJson, readme] = await Promise.all([
      ...
      fetchReadme(owner, name, customGithubToken),
    ]);
    ...
    // Enrich the profile response with description and stars before saving
    const enrichedProfile = {
      ...llmResponse,
      repoDescription: meta.description,
      repoStars: meta.stargazers_count,
    };
    ...
    try {
      ...
      const dbRepo = await prisma.repository.upsert({
        ...
        update: {
          url: meta.html_url,
          profileJson: JSON.stringify(enrichedProfile),
